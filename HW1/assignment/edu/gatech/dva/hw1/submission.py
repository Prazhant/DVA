...
class Graph:
    ...
    def add_edge(self, source: str, target: str)->None:
        ...
        t = (source, target)
        t2 = (target, source)
        if t not in self.edges and t2 not in self.edges:
            self.edges.append(t)
        # return NotImplemented
    ...
    def max_degree_nodes(self)->dict:
        ...
        or {'a': 22, 'b': 22}
        """
        c1 = Counter(x[0] for x in self.edges)
        c2 = Counter(x[1] for x in self.edges)
        c3 = c1+c2
        values = list(c3.values())
        max_value = max(values)
        count = values.count(max_value)
        ret = c3.most_common(n=count)
        di = dict(ret)
        return di

        # return NotImplemented

    def get_number_of_nodes_with_degree_greater_than_one(self)->int:
        c1 = Counter(x[0] for x in self.edges)
        c2 = Counter(x[1] for x in self.edges)
        c3 = c1 + c2
        values = list(c3.values())
        print(values)
        return len([x for x in values if x>1])
...
class  TMDBAPIUtils:
    ...
    def get_movie_cast(self, movie_id:str, limit:int=None, exclude_ids:list=None) -> list:
        ...
        connection.request("GET", "/3/movie/%s/credits?api_key=%s&language=en-US" %(movie_id,self.api_key))
        response = connection.getresponse()
        result = json.loads(response.read().decode())
        if "cast" not in result.keys():
            return {}
        cast = result.get("cast")
        cast = sorted(cast,key=lambda c: c['order'])
    ...
    def get_movie_credits_for_person(self, person_id:str, vote_avg_threshold:float=None)->list:
        ...
        connection = http.client.HTTPSConnection("api.themoviedb.org")
        connection.request("GET", "/3/person/%s/movie_credits?api_key=%s&language=en-US" % (person_id, self.api_key))
        response = connection.getresponse()
        result = json.loads(response.read().decode())
        movie_credits = result.get("cast")
        if vote_avg_threshold is not None:
            movie_credits = [i for i in movie_credits if (i['vote_average'] >= vote_avg_threshold)]
...
def build_base_graph():
       ...
       tmdb_api_utils = TMDBAPIUtils(api_key='418024add42472c963b8d917b15c69bb')

       movie_credits=tmdb_api_utils.get_movie_credits_for_person(2975,8.0)
       new_nodes_added = set()
       for movie in movie_credits:
           movie_id=movie["id"]
           cast=tmdb_api_utils.get_movie_cast(movie_id,3,exclude_ids=[2975])
           for cast_member in cast:
               new_node=(cast_member["id"],cast_member["original_name"])
               graph.add_node(str(cast_member["id"]),str(cast_member["original_name"]).replace(",",""))
               graph.add_edge(str(2975),str(cast_member["id"]))
               new_nodes_added.add(new_node)
       ...
       while i <= 1:
           ...
           for node in nodes:
               movie_credits = tmdb_api_utils.get_movie_credits_for_person(node[0], 8.0)
               for movie in movie_credits:
                   movie_cast_members = tmdb_api_utils.get_movie_cast(movie["id"],3,exclude_ids=[node[0]])
                   for cast_member in movie_cast_members:

                       new_node = (str(cast_member["id"]), cast_member["original_name"].replace(",",""))
                       if new_node not in graph.nodes:

                           graph.add_node(str(cast_member["id"]), cast_member["original_name"].replace(",",""))
                           if i == 0:
                               nodes_prev.add(new_node)
                       graph.add_edge(str(node[0]), str(cast_member["id"]))

           i+=1
       graph.write_edges_file()
       graph.write_nodes_file()
       print("no of nodes with degree greater than1 :", graph.get_number_of_nodes_with_degree_greater_than_one())
...
if __name__ == "__main__":
    ...
    # graph.add_node(id='3000', name='Tom Hanks')
    # graph.add_node(id='3001', name='Tom Cruise')
    # graph.add_node(id='3002', name='Meryll Streep')
    # graph.add_node(id='3003', name='J lo')
    # graph.add_node(id='3004', name='Emma Watson')
    #
    #
    # tmdb_api_utils = TMDBAPIUtils(api_key='418024add42472c963b8d917b15c69bb')
    #
    # graph.add_edge('2975','3000')
    # graph.add_edge('3000','3001')
    # graph.add_edge('3001','3000')
    # graph.add_edge('3001','2975')
    # graph.add_edge('3001', '3002')
    # graph.add_edge('2975', '3003')
    # graph.add_edge('3000', '3004')
    #
    # print(graph.max_degree_nodes())
    # assert(len(graph.max_degree_nodes())==3)
    # print(graph.total_edges())
    # assert (graph.total_edges()==6)
    # print(graph.total_nodes())
    # assert (graph.total_nodes() == 6)
    # print(graph.get_number_of_nodes_with_degree_greater_than_one())
    # assert(graph.get_number_of_nodes_with_degree_greater_than_one() ==3)
    #
    #
    ...
    # graph.write_nodes_file()
    #
    # # get movie cast default
    # ret = tmdb_api_utils.get_movie_cast(movie_id="464052")
    # print(len(ret))
    # assert (len(ret) == 147)
    # for c in ret:
    #     print(c)
    #
    # # get movie cast limit = 5
    # ret = tmdb_api_utils.get_movie_cast(movie_id="464052",limit=5)
    # print(len(ret))
    # assert (len(ret) == 5)
    # for c in ret:
    #     print(c)
    #
    # # get movie cast limit = 5 and exclude ids
    # ret = tmdb_api_utils.get_movie_cast(movie_id="717018", limit=3 , exclude_ids=[2975])
    # print(len(ret))
    # assert (len(ret) == 2)
    # for c in ret:
    #     print(c)
    #
    # # get movie cast with exclude ids
    # ret=tmdb_api_utils.get_movie_cast(movie_id="464052",exclude_ids=[41091,1253360,935])
    # print(len(ret))
    # assert(len(ret)==144)
    # for c in ret:
    #     print(c)
    #
    # #get movie credits without threshold vlue
    # credits = tmdb_api_utils.get_movie_credits_for_person(90633)
    # print(len(credits))
    # assert (len(credits) == 22)
    # for c in credits:
    #     print(c)
    # # get movie credits with threshold vlue
    # credits = tmdb_api_utils.get_movie_credits_for_person(90633,7.2)
    # print(len(credits))
